...
**Module 5 — FastAPI Development, L11** (built on L10, Security Basics)

A task and item API with a pytest suite. L11 supplied a Task CRUD API and asked
for an isolated test database and 8+ tests. L10 had already hardened an item
API with CORS restricted to named origins, a per-IP rate limit of 10 requests
per minute, and HTML sanitization. The module's exercises roll into one
project, so this is a single app: the L10 security layer now covers the L11
task endpoints too, and one database holds both tables.
...
pytest tests/ -v                   # the L11 test suite
...
| File | Contents |
|---|---|
| `app/main.py` | CORS, rate-limit, body-size-limit and HEAD middleware; error handlers; both routers |
...
| `app/models/item.py` | the `Item` ORM model (L10) |
| `app/schemas/task.py` | `TaskCreate`, `TaskPatch`, `TaskResponse` |
| `app/routers/tasks.py` | `/tasks` CRUD endpoints |
| `app/routers/items.py` | `ItemCreate`, `ItemResponse`, `GET /items`, `POST /items` (L10) |
| `tests/conftest.py` | in-memory test database; `client`, `db_session` and `deleted_mid_request` fixtures |
| `tests/helpers.py` | `call()` request helper, error-envelope and database read-back helpers |
...
| `verify_security.py` | 287 checks across 15 sections for the L10 defences |
...
| `requirements.txt` | dependencies; `pytest` is the only addition since L10 |
...
| `GET /items` | 200, list of items; optional `?name=` exact filter | 422 |
| `POST /items` | 201, the stored item | 422 |
...
## How the two lessons were combined
...
**119 passed** on the last run, in about a second, on Python 3.12.3 with
...
- **The rate limiter is reset around every test.** Its state lives in
  ...
  10 requests; removing it makes 108 of the 119 tests fail with 429.
...
**All 287 checks passed** on the last run. It uses a scratch database and
