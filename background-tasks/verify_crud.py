...
#
# POST /students now schedules a notification that sleeps 2 seconds, and
# TestClient waits for background tasks to finish before returning a response
# (it awaits the ASGI call directly — there is no socket to hand the bytes to
# and walk away from). This script creates students roughly six times, so
# leaving the delay alone turns a 3-second run into a 15-second one while
# testing nothing about notifications.
#
# Zero is the honest setting here: this file covers L6-L8 behaviour, and
# whether the response beats the work is verify_background_tasks.py's job,
# measured against a real server where the question is meaningful.
#
# LOG_DIR is redirected for the same reason — a regression run should not
# append to the activity log that ships as the exercise's evidence.
_SCRATCH = tempfile.mkdtemp(prefix="verify-crud-")
