...
It shows, in order:
  ...
  3. a POST made while limited is refused AND creates no row (checked in the
     database directly, since asking the API would spend another request)
...
try:
    ...
    # ----------------------------------------------------------------------
    out("\n[3] A POST while limited is refused and writes nothing")
    out("-" * 72)
    before = item_count()
    blocked = send(client, "POST", "/items", json={"name": "Should not be created"})
    after = item_count()
    if blocked is not None:
        raw_response("POST", "/items", blocked, {"Content-Type": "application/json"})
        out()
        expect("status", blocked.status_code, 429)
    out("  rows in the items table, read from the database directly:")
    out(f"    before the POST: {before}")
    out(f"    after the POST:  {after}")
    expect("no row was created", after, before)
    ...
    expect("the server logged three 429 responses", len(logged), 3)
