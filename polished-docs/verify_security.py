...
The L11 task endpoints share this security layer. tests/test_security.py
covers it on /tasks; this script stays focused on /items, where L10 built it,
plus the app-wide CORS and rate-limit behaviour.
...
should not write rows into the app.db a developer is using.
"""
...
from app.sanitization import sanitize_text  # noqa: E402
...
# L12 made price and category required. Every item body below carries these,
# so each check is refused (or accepted) for the reason its label gives.
PRICED = {"price": 9.99, "category": "tools"}
...
try:
    ...
    with unmetered():
        ...
        r = request(client, "POST", "/items", json={**PRICED, **clean}, expect=201, label="clean item is 201")
        ...
        for sent, expected in hostile_cases:
            r = request(client, "POST", "/items", json={**PRICED, **sent}, expect=201, label=f"hostile item {sent['name'][:20]!r} is 201")
    ...
    with unmetered():
        ...
        refusals = [
            ...
            ("an unknown field", {"name": "ok", "colour": "red"}),
            ("a misspelled field", {"nmae": "ok"}),
            # L12 fields
            ("a price of 0", {"name": "ok", "price": 0}),
            ("a negative price", {"name": "ok", "price": -1}),
            ("a price that is not a number", {"name": "ok", "price": "cheap"}),
            ("a category that is only markup", {"name": "ok", "category": "<i></i>"}),
            ("a null category", {"name": "ok", "category": None}),
        ]
        for label, body in refusals:
            r = request(client, "POST", "/items", json={**PRICED, **body}, expect=422, label=f"{label} is 422")
            if r is not None:
        ...
        r = request(client, "POST", "/items", json={**PRICED, "name": "x" * 100}, expect=201, label="a 100-character name is accepted")
    ...
    with unmetered():
        ...
        if r is not None:
            ...
            check("  listing exactly GET, POST, PATCH and DELETE", r.headers.get("access-control-allow-methods"), "GET, POST, PATCH, DELETE")
            check("  caching the answer for 600s", r.headers.get("access-control-max-age"), "600")

        # PATCH and DELETE arrived with the L11 task endpoints. PUT is still
        # refused, because nothing in the API accepts it.
        for method in ("PATCH", "DELETE"):
            r = preflight(ALLOWED, method)
            if r is not None:
                check(f"preflight: {method} (used by /tasks/{{id}}) is allowed", r.status_code, 200)
        r = preflight(ALLOWED, "PUT")
        if r is not None:
            check("preflight: PUT is refused", r.status_code, 400)
        ...
        for origin in (ALLOWED, EVIL, "null"):
            for method, path in (("GET", "/items"), ("GET", "/nope"), ("POST", "/items")):
                resp = send(client, method, path, headers={"Origin": origin}, json={**PRICED, "name": "x"} if method == "POST" else None)
    ...
    with fake_clock():
        ...
        request(client, "POST", "/items", json={**PRICED, "name": "Should not exist"}, expect=429, label="a POST while limited is 429")
        with unmetered():
    ...
    with fake_clock() as clock:
        ...
        mixed = [
            ...
            send(other, "POST", "/items", json={**PRICED, "name": ""}),
    ...
    with unmetered():
        ...
        # 50 KB stays under the request body limit, so it reaches validation.
        long_name = b'{"name": "' + b"x" * 50_000 + b'"}'
        r = request(client, "POST", "/items", content=long_name, headers=JSON, expect=422, label="a 50 KB invalid name is 422")
        if r is not None:
            check("  answered in under 2 KB, not 50 KB", len(r.content) < 2000, True)
            echoed = ((json_of(r, "long name") or {}).get("errors") or [{}])[0].get("input", "")
            check("  echoing a 200-character preview", (len(echoed), echoed.endswith("…")), (201, True))

        # Added with the request body limit (second round of probing): a body
        # this large is now refused before it is read into memory at all.
        huge = b'{"name": "' + b"x" * 5_000_000 + b'"}'
        r = request(client, "POST", "/items", content=huge, headers=JSON, expect=413, label="a 5 MB body is 413, refused before validation")
        if r is not None:
            check("  in the envelope", (json_of(r, "huge") or {}).get("error"), "PayloadTooLarge")
            check("  answered in under 2 KB", len(r.content) < 2000, True)
        ...
        # HEAD is listed wherever GET is answered (added in the second round).
        for method, path, allow in [("DELETE", "/items", "GET, HEAD, POST"), ("PUT", "/items", "GET, HEAD, POST"), ("POST", "/", "GET, HEAD")]:
        ...
        for label, name in [
            ...
            request(client, "POST", "/items", json={**PRICED, "name": name}, expect=422, label=f"a name of only {label} is 422")
        check("  none of them stored", count_now(), before)
        r = request(client, "POST", "/items", json={**PRICED, "name": "Wid\u200bget"}, expect=201, label="a real name containing a zero-width space is accepted")
        if r is not None:
    ...
    # Second round of edge-case probing: fixes that also apply to /items.
    with unmetered():
        client = client_from("10.0.0.16")

        # A deleted row's id is never handed out again. The API has no item
        # delete, so the row is removed directly, as someone cleaning up by
        # hand would.
        r = request(client, "POST", "/items", json={**PRICED, "name": "Newest"}, expect=201, label="create the newest item")
        newest = (json_of(r, "newest") or {}).get("id") if r is not None else None
        if newest is not None:
            with engine.begin() as connection:
                connection.execute(delete(Item).where(Item.id == newest))
            r = request(client, "POST", "/items", json={**PRICED, "name": "After"}, expect=201, label="create another after deleting it")
            after = (json_of(r, "after") or {}).get("id") if r is not None else None
            check("  which does not reuse the deleted id", after is not None and after > newest, True)

        head = send(client, "HEAD", "/items")
        get = send(client, "GET", "/items")
        if head is not None and get is not None:
            check("HEAD /items is answered like GET", head.status_code, get.status_code)
            check("  with no body", head.content, b"")
            check("  and the same Content-Length", head.headers.get("content-length"), get.headers.get("content-length"))
