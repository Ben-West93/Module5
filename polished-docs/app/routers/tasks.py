# exercises/polished-docs/app/routers/tasks.py
# L11 — Task endpoints
#
...
#   digits.
#
# L12 (Polished Documentation) added a summary, a Swagger docstring and a
# documented responses={} map to every endpoint, and moved the id type to
# app/ids.py so GET /items/{item_id} shares it.
...
from app.ids import SQLITE_MAX_INTEGER, plain_id  # noqa: F401  (SQLITE_MAX_INTEGER is imported by the tests)
...
TaskId = plain_id("task_id", "The task's id")

NOT_FOUND = {404: {"model": ErrorResponse, "description": "No task has that id"}}
CONFLICT = {409: {"model": ErrorResponse, "description": "Another task already has that title"}}
INVALID = {422: {"model": ErrorResponse, "description": "The body or id failed validation"}}
...
@router.post("", response_model=TaskResponse, status_code=status.HTTP_201_CREATED, summary="Create a task", responses={**CONFLICT, **INVALID})
def create_task(task: TaskCreate, db: Session = Depends(get_db)):
    """
    Create a task.

    - **title** (required, 1-200 characters): HTML tags are stripped and invisible
      characters removed; must still contain visible text. Must be unique.
    - **description** (optional, up to 500 characters, may be null): tags stripped.
    - **completed** (optional, default `false`).

    Unknown fields are refused. Returns **201** with the stored task, **409** if
    the title is taken, **422** for an invalid body.
    """
...
def list_tasks(db: Session = Depends(get_db)):
    """
    List every task, ordered by `id` (oldest first).

    Not paginated: every task is returned in one response.
    """
    return db.execute(select(Task).order_by(Task.id)).scalars().all()


@router.get("/{task_id}", response_model=TaskResponse, summary="Get one task", responses={**NOT_FOUND, **INVALID})
def get_task(task_id: TaskId, db: Session = Depends(get_db)):
    """
    Get a single task by `task_id`.

    The id must be plain digits between 1 and 2^63-1 (**422** otherwise).
    Returns **404** if no task has that id.
    """
    return get_task_or_404(db, task_id)


@router.patch("/{task_id}", response_model=TaskResponse, summary="Update part of a task", responses={**NOT_FOUND, **CONFLICT, **INVALID})
def patch_task(task_id: TaskId, task_data: TaskPatch, db: Session = Depends(get_db)):
    """
    Update only the fields sent; fields left out are unchanged.

    `title` and `completed` cannot be null; `description` can (clears it). An empty
    body changes nothing. Returns **404**, **409** (title taken) or **422**.
    """
...
@router.delete("/{task_id}", summary="Delete a task", responses={**NOT_FOUND, **INVALID})
def delete_task(task_id: TaskId, db: Session = Depends(get_db)):
    """
    Delete a task. Returns `{"message": "Task N deleted"}`.

    Deleting the same task again is a **404**. Ids are never reused.
    """
